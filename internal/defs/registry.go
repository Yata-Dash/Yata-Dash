...
func validateType(td TypeDef) error {
	...
	switch td.API.Kind {
	case "unit3d", "gazelle", "gazelle_json", "gazelle_json_cookie", "gazelle_games", "custom", "demo", "none":
	default:
		return fmt.Errorf("api.kind must be one of unit3d|gazelle|gazelle_json|gazelle_json_cookie|gazelle_games|custom|demo|none, got %q", td.API.Kind)
	}
	return nil
}
